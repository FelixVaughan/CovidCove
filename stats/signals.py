...
from stats.models import Last_time
import sys 

@receiver(post_save, sender=Last_time)
def check_time_table(sender, **kwargs): #checks to make sure that exactly one element is present in db
    if(len(sender.objects.all()) > 1):
        user_input = input("There appears to be more than one entry in 'Last_time' db. delete all but pk=1 (y/n)? ")
